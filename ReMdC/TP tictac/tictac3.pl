...
play(Table):-write('X turn. Press number of cell: '),read(M),move(Table,M,Tablea),show(Tablea),iimove(Tablea,Tableb),show(Tableb),play(Tableb).

playwithplayer(Table):-won(Table,x),write('X won').
playwithplayer(Table):-won(Table,o),write('O won').
playwithplayer(Table):-not(member(-,Table)),!,write('X=O').
playwithplayer(Table):-turn1(Table).
next(Table):-won(Table,x),write('X won').
next(Table):-won(Table,o),write('O won').
next(Table):-not(member(-,Table)),!,write('X=O').
next(Table):-turn2(Table).

turn1(Table):-write('X turn. Press number of cell: '),read(M),move(Table,M,Tablea),show(Tablea),next(Tablea).
turn2(Table):-write('O turn. Press number of cell: '),read(N),movesecond(Table,N,Tableb),show(Tableb),playwithplayer(Tableb).

show([A,B,C,D,E,F,G,H,I]) :- write([A,B,C]),nl,write([D,E,F]),nl,write([G,H,I]),nl,nl.

playgame:-show([-,-,-,-,-,-,-,-,-]),play([-,-,-,-,-,-,-,-,-]).

playboth:-show([-,-,-,-,-,-,-,-,-]),playwithplayer([-,-,-,-,-,-,-,-,-]).
...
movesecond([-,B,C,D,E,F,G,H,I], 1, [o,B,C,D,E,F,G,H,I]).
movesecond([A,-,C,D,E,F,G,H,I], 2, [A,o,C,D,E,F,G,H,I]).
movesecond([A,B,-,D,E,F,G,H,I], 3, [A,B,o,D,E,F,G,H,I]).
movesecond([A,B,C,-,E,F,G,H,I], 4, [A,B,C,o,E,F,G,H,I]).
movesecond([A,B,C,D,-,F,G,H,I], 5, [A,B,C,D,o,F,G,H,I]).
movesecond([A,B,C,D,E,-,G,H,I], 6, [A,B,C,D,E,o,G,H,I]).
movesecond([A,B,C,D,E,F,-,H,I], 7, [A,B,C,D,E,F,o,H,I]).
movesecond([A,B,C,D,E,F,G,-,I], 8, [A,B,C,D,E,F,G,o,I]).
movesecond([A,B,C,D,E,F,G,H,-], 9, [A,B,C,D,E,F,G,H,o]).